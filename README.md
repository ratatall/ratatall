# Hi, I'm Adilkhan (Addy)

**Software engineer and computer science student at Columbia University**


[LinkedIn](https://www.linkedin.com/in/adilkhan-orakbay/) · [Email](mailto:orakbay.adilkhan@gmail.com) · [LeetCode](https://leetcode.com/u/ratatall/)


## Experience highlights

- Led **5 engineers** building Unity-based VR games with motion-tracking gloves for elderly performance analysis.
- Built a smart-band application used by **150+ internal users**, with Firebase and Azure PostgreSQL powering real-time health data and analytics.
- Served as the primary developer across grant-funded software projects totaling **$100K+** and collaborated on a usability study involving **100+ patients**.
- Built a 3D learning platform for **150+ concurrent users** and reduced MySQL query time by **20%**.
- Developed Selenium automations that saved **10+ hours per week** while improving test coverage and execution speed.

## Technical toolkit

| Area | Technologies |
|---|---|
| **Languages** | C++, Java, Python, SQL, JavaScript, TypeScript |
| **Systems & backend** | LLVM, Go, Spring, Node.js, GraphQL, PostgreSQL, Docker, Linux |
| **Frontend & mobile** | React, Next.js, Flutter, three.js |
| **AI/ML & data** | PyTorch, pandas, NumPy, OpenCV, scikit-learn, SentenceTransformers, Azure OpenAI |
| **Cloud & platforms** | Azure, AWS, Firebase, OpenSearch |

## Selected work

| Project | What I built | Stack |
|---|---|---|
| **[TensorForge](https://github.com/ratatall/TensorForge)** | A C++20 tensor expression compiler with a custom frontend, typed IR, optimization passes, LLVM lowering, and ORC JIT execution. Elementwise fusion produced a **2.92x speedup** over the unfused JIT path and eliminated **2 MiB** of scratch memory in the measured workload. | C++20, LLVM, CMake |
| **[TalentLens](https://github.com/ratatall/talentlens)** | An evidence-first candidate search MVP with reviewable query interpretation, BM25/vector retrieval, reciprocal rank fusion, exact-source explanations, and reproducible evaluation tooling. | Next.js, TypeScript, OpenSearch, OpenAI API |
| **[SupportDesk](https://github.com/ratatall/supportdesk)** | A multi-tenant B2B support inbox with secure sessions, GraphQL workflows, PostgreSQL persistence, workspace isolation, and transactionally consistent audit history. | React, TypeScript, Go, GraphQL, PostgreSQL |

## A little more about me

- Dean's List at CityUHK for Semester B 2023/24 and Semester A 2024/25.
- Silver Medal at a GenAI Hackathon.
- Selected for the CityUHK Competitive Programming Training Team after placing in the top 10% of a selective competition.
- I originally planned to become an indie game developer - that interest still shapes how I think about interactive systems and product experience.

I'm always interested in ambitious engineering problems, especially where systems, AI, and real-world product constraints meet.
